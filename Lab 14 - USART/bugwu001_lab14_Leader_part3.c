 ...
 *  https://drive.google.com/file/d/1158aPh2ZsjfbWs6rm7X3Fn-7UvcG5M-f/view?usp=sharing
 */
...
int Lead_ToggleSMTick(int state){
    ...
    switch (state) { //State Transition
        case Start:
            ...
            break;
        case Toggle:
            if(USART_HasReceived(0)){state = Receive;}
            else{
                state = Transmit;
            }
            break;
        case Transmit:
            ...
            break;
        case Receive:
            if(counter > 3) {state = Toggle;}
            else{
                if(receive_complete == 0x00 && counter < 4) {state = Receive;}
                if(receive_complete == 0x01 && counter < 4) {state = Output;}                
            }
            break;
        case Output:
            state = Receive;
